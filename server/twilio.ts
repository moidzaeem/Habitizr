...
export async function sendHabitReminder(habit: any) {
  try {
    ...
    if (!user.stripeCustomerId) {
      console.error('Stripe customer ID missing for user:', user.id);
      return;
    }
    ...
    // Round difference to avoid small millisecond offsets
    const isOneDayLeft = Math.abs(diffInMs - msInOneDay) < (60 * 60 * 1000); // Within 1 hour of 24h mark
    let message;
    if (isOneDayLeft) {
      console.log('Subscription expires in exactly one day.');
      message = 'Your subscription is about to expire in 24 hours. Please renew to continue enjoying our services.';
    } else {
      console.log('Subscription has more than or less than one day left.');
      // Generate personalized message
      message = await generateReminderMessage(habit, stats, user, false);
    }
